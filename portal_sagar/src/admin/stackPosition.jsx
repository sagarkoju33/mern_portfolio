...
function StackPosition() {
  ...
  const stackPosition = portfolioData?.stackPositions || [];
    ...
    try {
      ...
      if (selectedItemForEdit) {
        response = await axios.post(
          `${BASE_URL}/api/portfolio/update-stackPosition`,
          {
            ...value,
        ...
        response = await axios.post(
          `${BASE_URL}/api/portfolio/add-stackPosition`,
          value
        );
      }
  ...
  return (
    <div className="text-white">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-5">
        {stackPosition.map((stackPosition, index) => {
          return (
            <div key={index} className="shadow border p-5 border-gray-400">
              <h1 className="text-md mt-3">Title : {stackPosition.title}</h1>
              <h1 className="text-sm">
                Description : {stackPosition.description}
              </h1>
              ...
              <div className="flex justify-end gap-5 mt-5">
                ...
                <button
                  className="bg-green-400 text-white px-5 py-2 rounded-md"
                  onClick={() => {
                    setSelectedItemForEdit(stackPosition);
                    setShowAddEditModal(true);
                  }}
                >
                  Edit
